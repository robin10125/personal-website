...
(function () {
  ...
  function render(L) {
    ...
    for (var rr = 0; rr < ROWS; rr++) {
      ...
      var col = L.rows[rr] ? rgb(L.rows[rr]) : "transparent";
      stops.push(col + " " + a + "%");
      stops.push(col + " " + b + "%");
    }
  ...
  // --- phase A: fade in the seeded diagonal (the first step of bars) --------
  var FADEIN_MS = 400;

  function fadeInSeeds() {
    return new Promise(function (resolve) {
      for (var f = 0; f < ROWS; f++) depositSeed(f);
      anim.style.opacity = "0";
      var start = null;
      function frame(ts) {
        if (start === null) start = ts;
        var p = Math.min(1, (ts - start) / FADEIN_MS);
        anim.style.opacity = String(p);
  ...
  // --- run the sequence ----------------------------------------------------
  // Fade in the seeded diagonal, then all-gather, then fade to black.
  fadeInSeeds()
    .then(function () {
      return wait(200);
    })
